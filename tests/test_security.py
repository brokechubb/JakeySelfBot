...
class TestSecurityValidator(unittest.TestCase):
    ...
    def test_validate_string_dangerous_inputs(self):
        ...
        for dangerous_string in dangerous_strings:
            ...
            self.assertFalse(
                is_valid, f"Dangerous string passed validation: {dangerous_string}"
            )
    ...
    def test_validate_crypto_symbol_safe(self):
        ...
        valid_symbols = ["BTC", "ETH", "DOGE", "USDT", "ADA", "DOT", "LINK", "UNI"]

        for symbol in valid_symbols:
            is_valid, error = self.validator.validate_cryptocurrency_symbol(symbol)
    ...
    def test_validate_currency_code_safe(self):
        ...
        valid_codes = ["USD", "EUR", "GBP", "JPY", "AUD", "CAD"]
    ...
    def test_validate_tip_command_safe(self):
        ...
        for recipient, amount, currency, message in valid_tips:
            is_valid, error = self.validator.validate_tip_command(
                recipient, amount, currency, message
            )
            self.assertTrue(
    ...
    def test_validate_tip_command_invalid(self):
        ...
        for recipient, amount, currency, message in invalid_tips:
            ...
            self.assertFalse(
                is_valid,
                f"Invalid tip command passed: {recipient}, {amount}, {currency}",
            )
    ...
    def test_validate_reminder_data_safe(self):
        ...
        for title, description, trigger_time in valid_reminders:
            is_valid, error = self.validator.validate_reminder_data(
